...
import { useAddChatMember, useRemoveChatMember } from "../api/useChatMembersMutations";

// 🚀 Описываем строгую структуру данных для каждого сотрудника в списке
interface EmployeeInChat {
  profileId: string;
  name: string;
  isInChat: boolean; // Флаг присутствия: состоит человек в этой теме или нет
}

// 🚀 Описываем строгие типы пропсов самого компонента диалога
interface ChatMembersDialogProps {
  currentChatId: string;
  employeesInOrg: EmployeeInChat[];
}

export const ChatMembersDialog = ({ currentChatId, employeesInOrg }: ChatMembersDialogProps) => {
              ...
              >
                ...
                {isAlreadyInChat ? (
                  ...
                  <Button 
                    ...
                    onClick={() => removeMember({ chatId: currentChatId, targetProfileId: employee.profileId })}
                  >
                    Исключить
                  </Button>
                ) : (
                  ...
                  <Button 
                    ...
                    onClick={() => addMember({ chatId: currentChatId, targetProfileId: employee.profileId })}
                  >
                    Добавить
                  </Button>
                )}
