...
async function checkManagePermission(
  ...
  if (!membership || membership.role !== OrgRole.RESPONSIBLE) {
    return {
      allowed: false,
      error: "У вас нет прав менеджера для управления участниками чата этой организации",
    }
  }
 ...
 */
export const addChatMemberAction = async ({ chatId, targetProfileId }: ChatMemberPayload) => {
  try {
    const session = await getSession()
    if (!session?.user) return { success: false, error: "Не авторизован" }

    // Проверяем права админа или локального RESPONSIBLE
    const check = await checkManagePermission(chatId, session.user.id, session.user.role)
    if (!check.allowed || !check.chat) return { success: false, error: check.error }
 ...
 */
export const removeChatMemberAction = async ({ chatId, targetProfileId }: ChatMemberPayload) => {
  try {
    const session = await getSession()
    if (!session?.user) return { success: false, error: "Не авторизован" }
 ...
 */
export const updateChatTitleAction = async ({ chatId, newTitle }: UpdateTitlePayload) => {
  try {
    const session = await getSession()
    ...
    if (!session?.user || session.user.role.toLowerCase() !== "admin") {
      return {
        success: false,
        error: "Доступ запрещен. Менять названия тем может только главный администратор.",
      }
    }
...
export const deleteChatAction = async ({ chatId }: DeleteChatPayload): Promise<ActionResponse> => {
  try {
    const session = await getSession()
    ...
    if (!session?.user || session.user.role.toLowerCase() !== "admin") {
      return {
        success: false,
        data: null,
        error: "Доступ запрещен. Удалять темы обсуждений может только администратор.",
      }
    }
