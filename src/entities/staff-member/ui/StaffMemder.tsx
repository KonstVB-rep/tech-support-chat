...
const StaffMemberCard = ({ staffMember }: { staffMember: StaffMemberWithProfile }) => {
  const userName = staffMember.profile.user.name || "Без имени"
  const userEmail = staffMember.profile.user.email || ""
  const imageUrl = staffMember.profile.imageUrl
      ...
      <div className="mt-4 transition-all duration-300 group-hover:scale-105">
        {imageUrl ? (
          ...
          <div className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-border bg-muted">
            <svg
              aria-labelledby={`staff-icon-${staffMember.id}`}
            ...
            >
              <title id={`staff-icon-${staffMember.id}`}>Профиль</title>
              <path
