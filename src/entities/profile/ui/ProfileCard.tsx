                    ...
                    {memberships.map((member: any) => (
                      <div key={member.id} className="flex items-start justify-between gap-4 border-b border-border/50 last:border-0 pb-2 last:pb-0">
                        <div className="flex flex-col">
                         {member.role === OrgRole.RESPONSIBLE ?  <Link href={`/organization/${member.organization.id}`} className="text-sm font-medium text-foreground hover:underline">{member.organization.name}</Link>: 
                            <span className="text-sm font-medium text-foreground">{member.organization.name}</span>
                         }
