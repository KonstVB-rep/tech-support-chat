...
export const GET = async (request: Request) => {
  try {
    ...
    // Создаем пустой объект-заглушку, который Next.js прокинет для получения контекста сокета
    const socketContext = {
      socket: (global as any).htmlServerResponse?.socket,
